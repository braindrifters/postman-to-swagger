...
const generateJSONSchema = (data) => {
    ...
    const processedSchema = (data instanceof Array) ? ({
        "type": "array",
        "items": (typeof data[0] === "string")?
            {
                "type": "string",
                "example": data[0]
            } :
            {
                "ref" : generateJSONSchema(data[0])
            }
...
const processReponse = (response) => {
    let schemaName = '',schemaType = 'object';
    if(response) {
        let responseData = JSON.parse(response.replace(/\n/g,""));
        schemaType = 'object'// (responseData instanceof Array) ? 'array':'object';
        schemaName = generateJSONSchema(responseData)
    }
    ...
    ${apiDef.method}:
      ...
      ${apiDef.response.length>0 ? apiDef.response.reduce((all, responseObj) => `${all}
        ${responseObj.code}:
          ...
          schema:
            ${responseObj.body.schemaType === 'array'
            ?`type: "array"
            items:
               $ref: "#/definitions/${responseObj.body.schemaName}"`
            :`$ref: "#/definitions/${responseObj.body.schemaName}"`
            }
...
definitions: ${Object.entries(schemaDef).reduce((all, [defKey, defValue]) => `${all}
  ${!(defValue instanceof Array) ?
  `${defKey}: 
    type : ${defValue.type}
    items: 
      $ref: "#/definitions/${defValue.items.ref}"` :
  `${defKey}:
    type: "object"
